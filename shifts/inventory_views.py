...
from .tool_extension_constants import (
    ...
    TOOL_EXTENSION_INNER_DIAMETER_CLAMPS,
    build_tool_extension_display_name,
...
def _arrival_bulk_row_validation_errors(row: dict, idx: int) -> list[str]:
    ...
    if category == "body_tool":
        ...
        return errs
    if category == "tool_extension":
        clamp = normalize_tool_extension_clamp(row.get("ext_clamp_type"))
        if not clamp:
        ...
        if tool_extension_needs_inner_diameter(clamp):
            if _to_decimal_or_none(row.get("ext_inner_diameter_mm")) is None:
                errs.append(
                    f"Строка {idx}: укажите внутренний диаметр Dвн (мм) для термо/боковой фиксации."
                )
        return errs
    if category == "insert":
...
def _arrival_search_ready(row: dict) -> bool:
    ...
    if category == "tool_extension":
        ...
        if ready and tool_extension_needs_inner_diameter(clamp):
            ready = _to_decimal_or_none(row.get("ext_inner_diameter_mm")) is not None
        return ready
    if category == "tap":
...
def _arrival_candidate_tools(row: dict, *, limit: int = 20) -> list[ToolItem]:
    ...
    elif category == "tool_extension":
        qs = qs.select_related("tool_extension_spec")
        clamp = normalize_tool_extension_clamp(row.get("ext_clamp_type"))
        if clamp:
            qs = qs.filter(tool_extension_spec__clamp_type=clamp)
        brand = (row.get("ext_brand") or "").strip()[:80]
        if brand:
            qs = qs.filter(tool_extension_spec__brand__iexact=brand)
...
_STOCK_INLINE_FIELD_LABELS = {
    ...
    "ext_inner_diameter_mm": "Внутренний диаметр Dвн",
    "size_label": "Размер",
    ...
    "ins_custom_type": "Свой тип",
    "ins_name": "Наименование",
    "ins_thread_size": "Размер резьбовой",
    "ins_thread_side": "Внутр./наруж.",
    "ins_thread_hand": "Лев./прав.",
    "ins_thread_pitch": "Шаг резьбы",
    "ins_grade": "Сплав",
    "ins_brand": "Бренд",
    "ins_iso": "ISO",
...
def _apply_stock_detail_filters(qs, *, category: str, params: dict, exclude: frozenset | None = None):
    ...
    if category == "end_mill":
        ...
        ext_clamp = normalize_tool_extension_clamp(g("ext_clamp_type"))
        if ext_clamp:
            qs = qs.filter(tool_extension_spec__clamp_type=ext_clamp)
        ext_brand = (g("ext_brand") or "").strip()
        if ext_brand:
            qs = qs.filter(tool_extension_spec__brand__iexact=ext_brand)
        ext_compat = (g("ext_compatible_parts") or "").strip()
        if ext_compat:
            qs = qs.filter(tool_extension_spec__compatible_parts__icontains=ext_compat)
        ...
        ext_inner = _to_decimal_or_none(g("ext_inner_diameter_mm"))
        if ext_inner is not None:
            qs = qs.filter(tool_extension_spec__inner_diameter_mm=ext_inner)
...
def _tool_extension_fields_from_row(row: dict) -> dict:
    clamp = normalize_tool_extension_clamp(row.get("ext_clamp_type")) or "collet"
    inner = _to_decimal_or_none(row.get("ext_inner_diameter_mm"))
    if not tool_extension_needs_inner_diameter(clamp):
        inner = None
    return {
        "brand": (row.get("ext_brand") or "").strip()[:80],
        "clamp_type": clamp,
...
def _find_tool_extension_tool_match(spec_fields: dict):
    return (
        ToolItem.objects.select_for_update()
        .filter(
            ...
            tool_extension_spec__inner_diameter_mm=spec_fields["inner_diameter_mm"],
        )
...
def _create_tool_extension_tool(quantity, spec_fields: dict) -> ToolItem:
    name = build_tool_extension_display_name(
        brand=spec_fields["brand"],
        clamp_type=spec_fields["clamp_type"],
        main_diameter_mm=spec_fields["main_diameter_mm"],
        overall_length_mm=spec_fields["overall_length_mm"],
        inner_diameter_mm=spec_fields["inner_diameter_mm"],
...
def inventory_view(request):
    ...
    if action == "update_tool_cell":
        ...
        if field == "main_diameter_mm":
            ...
            update_fields = ["main_diameter_mm", "updated_at"]
            if tool.category == "tool_extension" and getattr(tool, "tool_extension_spec", None):
                ex = tool.tool_extension_spec
                tool.name = build_tool_extension_display_name(
                    ...
                    inner_diameter_mm=ex.inner_diameter_mm,
        ...
        if not common_ok:
            ...
            if cat == "end_mill" and tool.end_mill_spec:
                ...
                if field == "ins_name":
                    ...
                    return JsonResponse({"ok": False, "error": "Поле не поддерживается."}, status=400)
            elif cat == "tool_extension" and getattr(tool, "tool_extension_spec", None):
                ex = tool.tool_extension_spec
                if field == "ext_brand":
                    ex.brand = (value_raw or "").strip()[:80]
                    ex.save(update_fields=["brand"])
                elif field == "ext_clamp_type":
                    ex.clamp_type = normalize_tool_extension_clamp(value_raw) or ex.clamp_type
                    if not tool_extension_needs_inner_diameter(ex.clamp_type):
                        ex.inner_diameter_mm = None
                        ex.save(update_fields=["clamp_type", "inner_diameter_mm"])
                    else:
                        ex.save(update_fields=["clamp_type"])
                ...
                elif field == "ext_inner_diameter_mm":
                    if not tool_extension_needs_inner_diameter(ex.clamp_type):
                        return JsonResponse(
                            {"ok": False, "error": "Dвн только для термо и боковой фиксации."},
                            status=400,
                        )
                    ex.inner_diameter_mm = _to_decimal_or_none(value_raw)
                    ex.save(update_fields=["inner_diameter_mm"])
                else:
                    return JsonResponse({"ok": False, "error": "Поле не поддерживается."}, status=400)
                tool.name = build_tool_extension_display_name(
                    brand=ex.brand,
        ...
        with transaction.atomic():
            for row in rows:
                ...
                if category == "end_mill":
                    ...
                    spec_fields = _tool_extension_fields_from_row(row)
                    if not spec_fields["clamp_type"] or spec_fields["main_diameter_mm"] is None:
                        continue
                    tool = _find_tool_extension_tool_match(spec_fields)
                    if tool:
                        tool.quantity += quantity
                        tool.save(update_fields=["quantity", "updated_at"])
                    else:
    ...
    ins_custom_type_raw = _sq("ins_custom_type")
    ins_name_raw = _sq("ins_name")
    ins_thread_size_raw = _sq("ins_thread_size")
    ins_thread_side_raw = _sq("ins_thread_side")
    ins_thread_hand_raw = _sq("ins_thread_hand")
    ins_thread_pitch_raw = _sq("ins_thread_pitch")
    ...
    ext_clamp_type_raw = _sq("ext_clamp_type")
    ...
    ext_inner_diameter_raw = _sq("ext_inner_diameter_mm")
    ...
    insert_custom_types = _distinct_text_values(_opt_qs("insert", "ins_custom_type"), "insert_spec__custom_type")
    ...
    tool_extension_compats = _distinct_text_values(
        _opt_qs("tool_extension", "ext_compatible_parts"), "tool_extension_spec__compatible_parts"
    )
    tool_extension_diameters = [
        _norm_stock_decimal_str(str(v))
        for v in _opt_qs("tool_extension", "ext_main_diameter_mm")
        .exclude(main_diameter_mm__isnull=True)
        .values_list("main_diameter_mm", flat=True)
        .distinct()
        .order_by("main_diameter_mm")
        if v is not None
    ]
    ...
    tool_extension_inner_diameters = [
        _norm_stock_decimal_str(str(v))
        for v in _opt_qs("tool_extension", "ext_inner_diameter_mm")
        .exclude(tool_extension_spec__inner_diameter_mm__isnull=True)
        .values_list("tool_extension_spec__inner_diameter_mm", flat=True)
        .distinct()
        .order_by("tool_extension_spec__inner_diameter_mm")
        if v is not None
    ]
    tool_extension_inner_diameters = [v for v in tool_extension_inner_diameters if v]
    ...
    ctx = {
        ...
        "filters": {
            ...
            "ins_custom_type": ins_custom_type_raw,
            "ins_name": ins_name_raw,
            "ins_thread_size": ins_thread_size_raw,
            "ins_thread_side": ins_thread_side_raw,
            "ins_thread_hand": ins_thread_hand_raw,
            "ins_thread_pitch": ins_thread_pitch_raw,
            ...
            "ext_clamp_type": ext_clamp_type_raw,
            ...
            "ext_inner_diameter_mm": _norm_stock_decimal_str(ext_inner_diameter_raw),
            "tool_material": tool_material,
        ...
        "insert_filter_options": {
            ...
            "custom_types": insert_custom_types,
            "grades": insert_grades,
            "brands": insert_brands,
            "isos": insert_isos,
        },
        "insert_kinds": INSERT_KINDS,
        "insert_custom_types": insert_custom_types,
        "insert_thread_sides": INSERT_THREAD_SIDES,
        "insert_thread_hands": INSERT_THREAD_HANDS,
        "insert_thread_sizes": INSERT_THREAD_SIZES,
        "insert_thread_pitch_options": INSERT_THREAD_PITCH_OPTIONS,
        ...
        "tool_extension_clamp_types": TOOL_EXTENSION_CLAMP_TYPES,
        "tool_extension_compat_labels": TOOL_EXTENSION_COMPAT_LABELS,
        "tool_extension_compat_placeholders": TOOL_EXTENSION_COMPAT_PLACEHOLDERS,
        "tool_extension_inner_diameter_clamps": sorted(TOOL_EXTENSION_INNER_DIAMETER_CLAMPS),
        "tool_extension_filter_options": {
